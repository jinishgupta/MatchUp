...
import logo from './assets/memory-icon.svg';
...
const HomePage = ({ 
  ...
  // Use Web3 context for authentication
  const { address, isConnected, disconnect } = useWeb3();
  
  console.log('🏠 HomePage: Component rendered with connection status:', { address, isConnected });
  
  if (!isConnected || !address) {
    console.log('ℹ️ HomePage: User not connected, returning null');
    return null;
  }

  const handleLogout = async () => {
    console.log('🚪 HomePage: User logging out...');
    // Disconnect wallet
    await disconnect();
    console.log('✅ HomePage: User logged out successfully');
  };

  const handleChallengeStart = (challengeInfo) => {
    console.log('🎯 HomePage: Daily challenge starting:', challengeInfo);
  ...
  const getLatestStats = () => {
    console.log('📊 HomePage: Fetching latest stats from localStorage...');
    ...
    console.log('📊 HomePage: Latest stats loaded:', stats);
    return stats;
  };
  ...
  useEffect(() => {
    console.log('🔄 HomePage: Setting up stats refresh listener...');
    
    const refreshStats = () => {
      console.log('🔄 HomePage: Refreshing stats...');
      setLocalStats(getLatestStats());
      console.log("✅ HomePage: Stats refreshed successfully");
    };
    ...
    // Clean up
    return () => {
      console.log('🧹 HomePage: Cleaning up stats refresh listener...');
    ...
    >
      ...
      <header className="p-4 flex justify-between items-center bg-slate-800 bg-opacity-70 backdrop-blur-sm">
        <div className="flex items-center">
          <img src={logo} alt="MatchUp" className="h-10 mr-3" />
      ...
      <footer className="mt-12 p-4 text-center">
        <p className="opacity-70 text-sm">© 2025 MatchUp | Powered by Sepolia ETH</p>
      </footer>
    </motion.div>
  );
};
